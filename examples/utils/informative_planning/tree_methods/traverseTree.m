function tree = traverseTree(tree, mu_parent, Sigma_parent, x_parent, y_parent, z_parent, sim, Sigma_rr, action_path)
    if nargin < 9
    ...
    if isfield(tree, 'branch')
        ...
        for k = 1:numel(tree.branch)
            ...
            tree.branch(k) = traverseTree(tree.branch(k), mu_parent, Sigma_updated, x_updated, y_updated, z_updated, sim, Sigma_rr, new_action_path);
        end
    else
