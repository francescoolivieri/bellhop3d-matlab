function ssp_est_final_plots(data, s, outpath)
    ...
    if nargin >= 3 && ~isempty(outpath)
        ...
        exportgraphics(fig, fullfile(outpath, 'ssp_poster_summary.png'), 'Resolution', 300, 'BackgroundColor','white');
        % TL fit diagnostic: compare measured vs predicted TL at measured positions
            ...
            if ~isempty(n_meas) && n_meas > 0
                ...
                % Style axes and legends to ensure white backgrounds and black text
                axs2 = findall(fig2, 'type', 'axes');
                for ax = axs2'
                    ax.Color = 'w'; ax.XColor = 'k'; ax.YColor = 'k'; ax.ZColor = 'k';
                    if isprop(ax, 'Title') && ~isempty(ax.Title), ax.Title.Color = 'k'; end
                    if isprop(ax, 'XLabel') && ~isempty(ax.XLabel), ax.XLabel.Color = 'k'; end
                    if isprop(ax, 'YLabel') && ~isempty(ax.YLabel), ax.YLabel.Color = 'k'; end
                    if isprop(ax, 'ZLabel') && ~isempty(ax.ZLabel), ax.ZLabel.Color = 'k'; end
                end
                legs2 = findall(fig2, 'type', 'legend');
                for lg = legs2', lg.TextColor = 'k'; lg.Color = 'w'; end
                exportgraphics(fig2, fullfile(outpath, 'tl_diagnostics.png'), 'Resolution', 300, 'BackgroundColor','white');
            end
